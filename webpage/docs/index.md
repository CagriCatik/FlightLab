# FlightLab

> 3D Printed RC Plane Design and Build Documentation

Welcome to **FlightLab** — a complete, structured documentation of the design, analysis, and fabrication process for a **3D printed RC aircraft**.

This documentation covers the entire workflow for developing a custom 3D printed RC plane, including:

* **Concept design and configuration**
* **Aerodynamic modeling and analysis**
* **Structural and load simulation**
* **Component integration**
* **3D printing and assembly process**

| Part | Topic                                 |
| ---- | ------------------------------------- |
| 1    | Concept and general design principles |
| 2    | Winglets and tail tip optimization    |
| 3    | Aerodynamic analysis and simulation   |
| 4    | Part slicing and export workflow      |
| 5    | Fuselage structural design            |
| 6    | Wing structural design                |
| 7    | Aileron structure and actuation       |
| 8    | Wing box and servo mount integration  |
| 9    | Tail and elevator servo mounts        |
| 10   | Access hatch                          |
| 11   | Tail and elevator design              |
| 12   | Splitting and modular part handling   |
| 13   | Slicer and printing tutorial          |

Add the following acknowledgment and inspiration section to the end of the document:

---

## Inspiration and References

This documentation was inspired in part by the comprehensive Drone Design design tutorials available from the **Dynamix Systems** and similar educational sources. In particular, the workflow and structural design methods outlined here drew conceptual influence from the video series available at:

**Reference:**
[3D Printed RC Plane Design Series – YouTube Playlist](https://www.youtube.com/playlist?list=PLQpp11UVG0BpugWGMypDLL-O1jd6pH7je)

This playlist provided valuable insight into iterative design, modular assembly, and 3D printing considerations for lightweight Drone Design structures. It served as an educational foundation for refining both the design logic and documentation approach used throughout the **FlightLab** project.


<div class="stl-viewer"
     data-src="assets/models/part.stl"
     data-grid="false"
     data-autorotate="true"
     data-color="#9d1313ff"
     data-metalness="0.95"
     data-roughness="0.95">
</div>

---

<div class="stl-viewer"
     data-src="assets/models/part.stl"
     data-grid="false"
     data-autorotate="true"
     data-color="#9d1313ff"
     data-metalness="0.95"
     data-roughness="0.95">
</div>

---

<div class="stl-viewer"
     data-src="assets/models/part.stl"
     data-grid="false"
     data-autorotate="true"
     data-color="#9d1313ff"
     data-metalness="0.95"
     data-roughness="0.95">
</div>

<script type="importmap">
{
  "imports": {
    "three": "javascripts/vendor/three.module.js"
  }
}
</script>
<script type="module" src="javascripts/stl-viewer.module.js"></script>
